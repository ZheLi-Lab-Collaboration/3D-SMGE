...
def smiles_to_xyz(smiles, num_conf=1, ob_gen3D_option='best'):
    # initialize obmol
    smiles = "CC(C1=CC=C(OC)C(OC)=C1)=O"
    ...
    print("num--------", atomic_numbers_fg)
    print("coord--------", conformer_coordinates)
    print("select-----------", conformer_coordinates[0][2])
    print("type-----------", type(conformer_coordinates))

    return atomic_numbers_fg, conformer_coordinates

# The input format is extract in xyz
# def smiles_to_xyz():
#     import re
#     atom_atomic_number_dict = {"H": 1, "C": 6, "N": 7, "O": 8, "F": 9, "Cl": 17, 'S': 16}
#     atomic_numbers_fg = []
#     conformer_coordinates = []
#     with open("a.xyz", "r", encoding="utf8") as r:
#         lines = r.readlines()
#         lines = lines[2:]
#
#         for line in lines:
...
#                 atom_coordinate = []
#                 atom = line.strip()[0]
#                 atomic_numbers_fg.append(atom_atomic_number_dict[atom])
#                 c1 = np.float(re.split(r"[ ]+", line)[1])
#                 c2 = np.float(re.split(r"[ ]+", line)[2])
#                 c3 = np.float(re.split(r"[ ]+", line)[3])
#                 atom_coordinate.append(c1)
#                 atom_coordinate.append(c2)
#                 atom_coordinate.append(c3)
#                 conformer_coordinates.append(atom_coordinate)
#                 print(atom)
...
# specific site
def specific_site_pdb(pdb_path):
    specific_site_atom_ind = []
    temp_specific_site_atom_ind = []
    conformer_coordinates = []
    atomic_numbers_fg = []
    specific_site = []
    ...
    with open(pdb_path) as f:
        ...
        # print("lones", lines)
        for line in lines:
            if (line.strip()).split()[0] == "HETATM" and (line.strip()).split()[-1] != "H":
                specific_site_atom_ind.append((line.strip()).split()[-1])
                temp_arr.append(np.float(re.split(r"[ ]+", line)[5]))
                temp_arr.append(np.float(re.split(r"[ ]+", line)[6]))
                temp_arr.append(np.float(re.split(r"[ ]+", line)[7]))
                conformer_coordinates.append(temp_arr)
                print(temp_arr)
                temp_arr = []

        for i in range(len(specific_site_atom_ind)):
            specific_site_atom_ind_ = specific_site_atom_ind[i].strip("*")
            temp_specific_site_atom_ind.append(specific_site_atom_ind_)
            specific_site_ = list(specific_site_atom_ind[i])

            if len(specific_site_) != 2:
                specific_site.append(i)

        for atom in temp_specific_site_atom_ind:
            atomic_numbers_fg.append(atom_atomic_number_dict[atom])

    print(specific_site_atom_ind)
    print(temp_specific_site_atom_ind)
    print(conformer_coordinates)
    print(atomic_numbers_fg)
    print(specific_site)

    return atomic_numbers_fg, conformer_coordinates, specific_site

def specific_site_mol2(mol2_path):
    # store
    specific_site_atom_ind = []
    temp_specific_site_atom_ind = []
    conformer_coordinates = []
    ...
    with open(mol2_path) as f:
        ...
        for i in range((lines.index("@<TRIPOS>ATOM\n")+1), lines.index("@<TRIPOS>BOND\n")):
            # print(lines[i])
            # print(list((lines[i].strip()).split()[1])[0])
            if list((lines[i].strip()).split()[1])[0] != "H":
                specific_site_atom_ind.append((lines[i].strip()).split()[1])
                # print(specific_site_atom_ind)
                # print((lines[i].strip()).split())
                temp_arr.append(np.float(re.split(r"[ ]+", lines[i])[3]))
                temp_arr.append(np.float(re.split(r"[ ]+", lines[i])[4]))
                temp_arr.append(np.float(re.split(r"[ ]+", lines[i])[5]))
                conformer_coordinates.append(temp_arr)
                # print(temp_arr)
                temp_arr = []

        for j in range(len(specific_site_atom_ind)):
            specific_site_atom_ind_ = (list(specific_site_atom_ind[j]))[0]
            temp_specific_site_atom_ind.append(specific_site_atom_ind_)
            # print(temp_specific_site_atom_ind)
            if (list(specific_site_atom_ind[j]))[-1] != "*":
                specific_site.append(j)
                # print("3333", specific_site)
...
def normal_pdb(pdb_path):
    ...
    with open(pdb_path) as f:
        ...
        for line in lines:
            if (line.strip()).split()[0] == "HETATM" and (line.strip()).split()[-1] != "H":
                atom_ind.append((line.strip()).split()[-1])
                temp_arr.append(np.float(re.split(r"[ ]+", line)[5]))
                temp_arr.append(np.float(re.split(r"[ ]+", line)[6]))
                temp_arr.append(np.float(re.split(r"[ ]+", line)[7]))
                conformer_coordinates.append(temp_arr)
...
def normal_mol2(mol2_path):
    ...
    with open(mol2_path) as f:
        ...
        for atom in specific_site_atom_ind:
            atom = list(atom)[0]
            print(atom)
            atomic_numbers_fg.append(atom_atomic_number_dict[atom])
    #
...
def update_dict(d, d_upd):
    ...
    Args:
        ...
        d_upd (dict of numpy.ndarray): dictionary with origin values for updating
    '''
    for key in d_upd:
        if key not in d:
            d[key] = d_upd[key]
...
def get_random_walk(mol_dict, stop_token=10, seed=None):
    ...
    order = [0]  # the origin ordering always starts with the first atom (closest to com)
...
def collate_atoms(mol_dicts,
    ...
    Returns:
        dict[str->torch.Tensor]: mini-bat+ch of atomistic systems
    '''
...
def generate_molecules(amount,
    ...
    # atomic_numbers_fg determine the sequence of atomic numbers of the core skeleton


    if genMode == "mode1":
        if inputFormat == "pdb":
            atomic_numbers_fg, coordinates_fg = normal_pdb(file3D_path)
    ...
    """The penultimate atom is set to stop token"""
    stop_token = all_types[-1]
    """Set stop_token to focus_token"""
    focus_token = stop_token
    print("1.amout", amount)
    ...
    """Initialize the tensor of atomic numbers"""
    """ amout is the amount generated, and max_length is the maximum length """
    ...
    print("1.atom_numbers", atom_numbers)
    print("1.atom_numbers size", atom_numbers.size())
    ...
    # the first two is the focus token and origin token
    if genMode == "mode2":
        print("have_finished_input", have_finished_input)
        have_fininshed = list(map(lambda x: int(x) + 2, have_finished_input))
        print("have finished + 2 ", have_fininshed)
        for i in range(len(have_fininshed)):
            atoms_unfinished[:, [have_fininshed[i]]] = 0

    print("atoms_unfinished:", atoms_unfinished)
    print("atoms_unfinished size", atoms_unfinished.size())
    """-------------------------------------------------------"""
    ...
    results = {}
    # create short na0me for function that pulls results from gpu and removes
    #  the start and current tokens (first two entries)
    s = lambda x: x[:, n_tokens:].detach().cpu().numpy()
    print("2.amout", amount)
    ...
    for i in range(n_tokens+len(atomic_numbers_fg), max_length):
        ...
        while torch.sum(unfinished) > 0:
            ...
            # proper type for which we can sample a origin position)
        ...
        amount = torch.sum(unfinished)
        print("3.amount", amount)
        if i >= max_length-1 or amount == 0:
            break

        ### 2nd Part ###
        # sample origin position given the type of the next atom
        ...
        # sample positions of origin atoms using the calculated pdfs over grid positions
