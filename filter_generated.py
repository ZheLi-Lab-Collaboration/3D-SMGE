...
if __name__ == '__main__':
    ...
    for n_atoms in res:
        ...
        if 'disconnected' in args.filters:
            ...
            for i in range(len(all_pos)):
                ...
                if np.any(dists) < 0.3:  # check if any two atoms are closer than 0.3 A
                    valid[i] = 0  # mark current molecule as invalid 这样就是标记出 invaild 的分子 这样分子为 0
        ...
        # duplicate_count is the number of repetitions per structure
        duplicate_count = np.zeros(n_mols, dtype=int)
        # duplicating contains -1 for original structures and the id of the duplicated original structure for duplicates
        ...
        n_of_types = [np.sum(all_numbers == i, axis=1) for i in [6, 7, 8, 9, 16, 17, 1]]
        stats_new = np.stack(
