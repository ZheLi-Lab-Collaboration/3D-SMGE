...
def get_parser():
    ...
    cmd_parser.add_argument('--batch_size', type=int,
                            help='Mini-batch size for training and prediction '
                                 '(default: %(default)s)',
                            default=5)
    # Reduce the number of batch random placement traces instead of full traces
    ...
    train_parser.add_argument('--pretrained_path',
                              help='Start training from the pre-trained model at the '
                                   ...
                                   'best loss and learning rate and create origin split)',
                              default=None)
    ...
    # The number of molecules that are generated
    ...
    gen_parser.add_argument('--chunk_size', type=int,
                            help='The size of mini batches during generation '
                                 '(default: %(default)s)',
                            default=1000)
    # How many atoms per molecule at most
    ...
    gen_parser.add_argument('--file_name', type=str,
                            ...
                            default='generated')
    # A file store of the maximum length of unfinished molecules sampled
...
def generate(args, train_args, model, device, gen_):
...
def main(args):
    ...
    if args.mode == 'train':
        ...
        # generate(args, train_args, model, device)
