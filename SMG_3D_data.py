...
from SMG_3D_preprocess_dataset import preprocess_dataset


class SMG_3Dgen(DownloadableAtomsData):
    """ SMG_3D benchmark dataset for organic molecules with up to nine non-hydrogen atoms
        from {C, O, N, F, S Cl}.

        This class adds convenience functions to download SMG_3D from figshare,
        ...
        Args:
            path (str): path to directory containing SMG_3D database
            ...
            remove_invalid (bool, optional): if True SMG_3D molecules that do not pass the
    ...
    atom_types_valence = [1, 4, 3, 2, 1, 2, 1]  # valence constraints of the atom types
    ...
    def __init__(self, path, subset=None, download=True, precompute_distances=True,
                 remove_invalid=True):
        self.path = path
        self.dbpath = os.path.join(self.path, f'SMG_3Dgen.db')
    ...
    def _download(self):
        ...
        if not os.path.exists(self.dbpath):
            SMG_3D_path = os.path.join(self.path, f'generated_molecules.db')
            if not os.path.exists(SMG_3D_path):
                works = works and self._load_data()
            works = works and self._preprocess_SMG_3D()
        return works
    ...
    def _load_data(self):
        ...
        with connect(os.path.join(self.path, 'generated_molecules.db')) as con:
    ...
    def _preprocess_SMG_3D(self):
        # try to download pre-computed list of invalid molecules
        logging.info('Downloading pre-computed list of invalid SMG_3D molecules...')
        raw_path = os.path.join(self.path, 'SMG_3D_invalid.txt')
        ...
        SMG_3D_db = os.path.join(self.path, f'generated_molecules.db')
        valence_list = \
            np.array([self.available_atom_types, self.atom_types_valence]).flatten('F')
        preprocess_dataset(datapath=SMG_3D_db, valence_list=valence_list,
