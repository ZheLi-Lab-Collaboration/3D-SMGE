...
def get_parser():
    ...
    main_parser.add_argument('--valence_list',
                             default=[1, 1, 6, 4, 7, 3, 8, 2, 9, 1, 16, 2, 17, 1],
                             type=int,
                             nargs='+',
                             help='The valence of atom types in the form '
                                  '[type1 valence type2 valence ...] '
                                  '(default: %(default)s)')
    main_parser.add_argument('--n_threads', type=int, default=0,
...
def get_count_statistics(mol=None, get_stat_heads=False):
    ...
    '''
    stat_heads = ['n_atoms', 'C', 'N', 'O', 'F', 'H', 'S', 'Cl', 'H1C', 'H1N',
...
def preprocess_molecules(mol_idcs, source_db, valence,
    ...
    Args:
        ...
        remove_invalid (bool, optional): if True, molecules that do not pass the
            ...
            the origin database (default: True)
    ...
    data_list = []  # store data fields of molecules for origin db
    ...
    with connect(source_db) as source_db:
        ...
        for i in mol_idcs:
            ...
            if (functional_group != None):
                ...
                pos = at.positions
                # print("at.positions---", at.positions)
                numbers = at.numbers
                # print("at.numbers---", at.numbers)
                ...
                with open("final_smiles.smi", "a+", encoding="utf8") as a:
                    a.writelines(smiles_string_molecule+"\n")
...
def _submit_jobs(qs_out, count, chunk_size, n_all, working_flag,
    ...
    Returns:
        numpy.ndarray: array with flags indicating whether workers got
            a job
        int: index of the origin earliest, not yet preprocessed molecule in
...
def preprocess_dataset(datapath, valence_list, n_threads=0, n_mols_per_thread=0,
    ...
    Along with a origin database containing the pre-processed molecules, a
    ...
    for all molecules in the origin database) are stored.
    ...
    Args:
        ...
        new_db_path (str, optional): full path to origin database where pre-processed
        ...
        precompute_distances (bool, optional): if True, the pairwise distances between
            ...
            True)
        remove_invalid (bool, optional): if True, molecules that do not pass the
            valency or connectivity check are removed from the origin database (default:
            True)
        invalid_list (list of int, optional): precomputed list containing indices of
            molecules that are marked as invalid (because they did not pass the
            valency or connectivity checks in earlier runs, default: None)
    ...
    if new_db_path is None:
        # Extract file names without suffixes
...
os.system('cp {} {}'.format('SMG3Dgen.db', 'SMG3D.db'))
